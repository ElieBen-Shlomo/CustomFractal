...
firstIndexOfElementSatisfyingCondition (x:xs) condition = if condition x then 1 else firstIndexOfElementSatisfyingCondition xs condition

-- firstIndexOfElementSatisfyingCondition :: [Complex] -> TruthCondition -> Int -> Int
-- firstIndexOfElementSatisfyingCondition list condition n = snd $ headOrMax $ filteredList 
--     where
--     filteredList = filter (\(z, n) -> condition z) enumeratedList
--         where
...
colouredPoints = [ (shiftX +factor*x/widthDensity, shiftY + factor*y/heightDensity, intToColour $ iterations x y 1)|
